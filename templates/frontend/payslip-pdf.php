<style>
    .payslip-container {
        font-family: Arial, sans-serif;
        max-width: 800px;
        margin: 0 auto;
        padding: 20px;
    }
    .payslip-header {
        text-align: center;
        margin-bottom: 30px;
        border-bottom: 2px solid #333;
        padding-bottom: 10px;
    }
    .payslip-company {
        font-size: 24px;
        font-weight: bold;
        margin-bottom: 5px;
    }
    .payslip-title {
        font-size: 18px;
        margin-bottom: 5px;
    }
    .payslip-period {
        font-size: 14px;
    }
    .payslip-info {
        display: flex;
        justify-content: space-between;
        margin-bottom: 30px;
    }
    .payslip-employee, .payslip-details {
        width: 48%;
    }
    .payslip-section-title {
        font-weight: bold;
        border-bottom: 1px solid #ddd;
        padding-bottom: 5px;
        margin-bottom: 10px;
    }
    .payslip-row {
        display: flex;
        justify-content: space-between;
        margin-bottom: 5px;
    }
    .payslip-label {
        font-weight: bold;
    }
    .payslip-table {
        width: 100%;
        border-collapse: collapse;
        margin-bottom: 30px;
    }
    .payslip-table th, .payslip-table td {
        border: 1px solid #ddd;
        padding: 8px;
        text-align: left;
    }
    .payslip-table th {
        background-color: #f5f5f5;
    }
    .payslip-total {
        text-align: right;
        font-size: 18px;
        font-weight: bold;
        margin-top: 20px;
        padding-top: 10px;
        border-top: 2px solid #333;
    }
    .payslip-footer {
        margin-top: 50px;
        display: flex;
        justify-content: space-between;
    }
    .payslip-signature {
        width: 40%;
        text-align: center;
    }
    .payslip-signature-line {
        margin-top: 50px;
        border-top: 1px solid #333;
        padding-top: 5px;
    }
</style>

<div class="payslip-container">
    <div class="payslip-header">
        <?php if (!empty($settings['company_logo'])) : ?>
            <img src="<?php echo esc_url($settings['company_logo']); ?>" alt="<?php echo esc_attr($settings['company_name']); ?>" style="max-height: 80px; max-width: 200px; margin-bottom: 10px;">
        <?php endif; ?>
        <div class="payslip-company"><?php echo esc_html($settings['company_name']); ?></div>
        <div class="payslip-title"><?php _e('SALARY SLIP', 'employee-management-system'); ?></div>
        <div class="payslip-period"><?php echo esc_html($month_name . ' ' . $salary->year); ?></div>
    </div>
    
    <div class="payslip-info">
        <div class="payslip-employee">
            <div class="payslip-section-title"><?php _e('Employee Details', 'employee-management-system'); ?></div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Name:', 'employee-management-system'); ?></div>
                <div><?php echo esc_html($employee->name); ?></div>
            </div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Employee ID:', 'employee-management-system'); ?></div>
                <div><?php echo esc_html($employee->id); ?></div>
            </div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Job Title:', 'employee-management-system'); ?></div>
                <div><?php echo esc_html($employee->job_title); ?></div>
            </div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Joining Date:', 'employee-management-system'); ?></div>
                <div><?php echo esc_html(date_i18n(get_option('date_format'), strtotime($employee->joining_date))); ?></div>
            </div>
        </div>
        
        <div class="payslip-details">
            <div class="payslip-section-title"><?php _e('Payment Details', 'employee-management-system'); ?></div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Payment Date:', 'employee-management-system'); ?></div>
                <div>
                    <?php if (!empty($salary->payment_date)) : ?>
                        <?php echo esc_html(date_i18n(get_option('date_format'), strtotime($salary->payment_date))); ?>
                    <?php else : ?>
                        <?php _e('Pending', 'employee-management-system'); ?>
                    <?php endif; ?>
                </div>
            </div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Payment Status:', 'employee-management-system'); ?></div>
                <div>
                    <?php echo $salary->payment_status === 'paid' ? __('Paid', 'employee-management-system') : __('Pending', 'employee-management-system'); ?>
                </div>
            </div>
            <div class="payslip-row">
                <div class="payslip-label"><?php _e('Payment Method:', 'employee-management-system'); ?></div>
                <div><?php _e('Bank Transfer', 'employee-management-system'); ?></div>
            </div>
        </div>
    </div>
    
    <table class="payslip-table">
        <thead>
            <tr>
                <th><?php _e('Description', 'employee-management-system'); ?></th>
                <th><?php _e('Amount', 'employee-management-system'); ?></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td><?php _e('Base Salary', 'employee-management-system'); ?></td>
                <td><?php echo esc_html($settings['currency_symbol'] . number_format_i18n($salary->base_salary, 2)); ?></td>
            </tr>
            <?php if ($salary->bonus > 0) : ?>
                <tr>
                    <td><?php _e('Bonus', 'employee-management-system'); ?></td>
                    <td><?php echo esc_html($settings['currency_symbol'] . number_format_i18n($salary->bonus, 2)); ?></td>
                </tr>
            <?php endif; ?>
            <?php if ($salary->deduction > 0) : ?>
                <tr>
                    <td><?php _e('Deduction', 'employee-management-system'); ?></td>
                    <td>-<?php echo esc_html($settings['currency_symbol'] . number_format_i18n($salary->deduction, 2)); ?></td>
                </tr>
            <?php endif; ?>
        </tbody>
        <tfoot>
            <tr>
                <th><?php _e('Total', 'employee-management-system'); ?></th>
                <th><?php echo esc_html($settings['currency_symbol'] . number_format_i18n($salary->total_salary, 2)); ?></th>
            </tr>
        </tfoot>
    </table>
    
    <?php if (!empty($salary->notes)) : ?>
        <div class="payslip-notes">
            <div class="payslip-section-title"><?php _e('Notes', 'employee-management-system'); ?></div>
            <p><?php echo esc_html($salary->notes); ?></p>
        </div>
    <?php endif; ?>
    
    <div class="payslip-footer">
        <div class="payslip-signature">
            <div class="payslip-signature-line"><?php _e('Employee Signature', 'employee-management-system'); ?></div>
        </div>
        <div class="payslip-signature">
            <div class="payslip-signature-line"><?php _e('Authorized Signature', 'employee-management-system'); ?></div>
        </div>
    </div>
</div>